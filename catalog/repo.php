                ...
                <table>
                    ...
                    <?php foreach($catalogStocks as $catalogStock){ ?>
                        <tr <?php if($check_item != $catalogStock['fab_title']){?>style="border-top:3px #FFD382 solid;"<?php } ?>>
                            ...
                            <td <?php if($catalogStock["amount"] < $catalogStock['standard_lv']){ ?> style="background-color:#FFBFFF;color:red;font-size:1.5em;" <?php }else{ ?> style="background-color:#BFFFFF;" <?php } ?>>
                                <?php echo $catalogStock['amount'];?></td><?php $sum_0 += (int)$catalogStock["amount"];?>
                            <td <?php if($catalogStock["lot_num"] < $half_month){ ?> style="background-color:#FFBFFF;color:red;" data-toggle="tooltip" data-placement="bottom" title="有效期限小於：<?php echo $half_month;?>" <?php } ?>>
