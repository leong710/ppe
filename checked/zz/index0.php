<?php
    ...
    // 先給預設值
    $sort_site_id = $site_id = $_SESSION[$sys_id]["site_id"];
    $sort_user_id = $user_id = $_SESSION[$sys_id]["id"];
    // 今年年份
    $today_year = date('Y');
    // 半年分界線
        if(date('m') <= 6 ){
    ...
    $mySite = show_site($list_setting);             // 調閱我的Site
...
<body>
    <div class="TOP">
        <div class="col-12">
            ...
            <div class="row justify-content-center">
                <div class="col-11 rounded p-3" style="background-color: rgba(255, 255, 255, .8);">
                    <!-- 歷史點檢紀錄表頭 -->
                    <div class="row px-2">
                        <div class="col-md-6">
                            <div>
                        ...
                        <div class="col-md-6 text-end">
                            <div>
                                <?php if($_SESSION[$sys_id]["role"] <= 0){ ?>
                                    <button type="submit" id="checkItem" class="btn btn-sm btn-xs btn-success">checkItem</button>
                                <?php } ?>
                            </div>
                        </div>
                    </div>
    ...
    <div class="col-12">
        <div class="row justify-content-center">
            ...
            <!-- unblock設在這裡 -->
            <div class="col_xl_11 col-11 rounded mb-3 unblock" id="checkList" style="background-color: #D4D4D4;">                    
                <div class="col-12 justify-content-center rounded" >
                    <form action="store.php" method="post">
                        <!-- 文件表頭 -->
                        <div class="row px-2">
                            <div class="col-md-6">
                                <h3>所屬Site區域衛材儲存量總表for年檢用</h3>
                            </div>
                            <div class="col-md-6 text-end">
                                <?php if(isset($_SESSION[$sys_id])){
                                    if(count($stocks) > 0){ ?>
                                        <?php if($_SESSION[$sys_id]["role"] <= 2){ ?>
                                            ...
                                            <?php }else{
                                                echo "<snap style='color: red;'>** 只限siteUser才能完成年檢動作!</snap>";
                                         } ?>
                                    <?php }else{
                                        echo "<snap style='color: red;'>** 請先建立衛材品項，再來完成年檢動作!</snap>";
                                    }?>
                                <?php } ?>
                            </div>
                        </div>
                        <div class="row px-2 py-0">
                            ...
                            <div class="col-md-6">
                                <div>
                                    Site廠區：<?php echo $mySite["site_title"];?></br>
                                ...
                                <tbody>
                                    ...
                                    <?php foreach($stocks as $stock){ ?>
                                        <tr <?php if($check_item != $stock['fab_title']){?>style="border-top:3px #FFD382 solid;"<?php } ?>>
                                            <td style="font-size: 6px;"><?php echo $stock['id'];?></td>
                                            <td style="text-align: left;"><?php echo $stock['fab_title'];?></td>
                                            <td style="text-align: left;"><?php echo $stock['site_title']."_".$stock['local_title'];?></td>
                                            <td style=""><?php echo $stock['cate_title'];?></td>
                                            <td style="text-align: left;"><?php echo $stock['catalog_title'];?></td>
                                            <td style=""><?php echo $stock['standard_lv'];?></td>
                                            <td style=""><?php echo $stock['amount'];?></td>
                                            <td style="text-align: left;"><?php echo $stock['remark'];?></td>
                                            <td style="font-size: 12px;"><?php echo $stock['lot_num'];?></td>
                                            <td style="font-size: 12px;"><?php echo $stock['po_num'];?></td>
                                            <td style="text-align: left;"><?php echo $stock['d_remark'];?></td>
                                            <td style="font-size: 12px;" title="最後編輯: <?php echo $stock['cname'];?>"><?php echo $stock['updated_at'];?></td>
                                        </tr>
                                        <?php 
                                            $check_item = $stock['fab_title'];
                                            // 因為remark=textarea會包含html符號，必須用strip_tags移除html標籤
                                            $s_remark = strip_tags($stock["remark"]);
                                            $s_d_remark = strip_tags($stock["d_remark"]);
                                            // 因為remark=textarea會包含換行符號，必須用str_replace置換/n標籤
                                            $s_remark = str_replace("\r\n", " ", $s_remark);
                                            $s_d_remark = str_replace("\r\n", " ", $s_d_remark);
                        ...
                        <!-- 這個工作是將stocks逐筆轉成array，以便進行鋪陳和存檔 -->
                            ...
                            <input type="hidden" name="site_id" value="<?php echo $stock["site_id"];?>">
                            <input type="hidden" name="user_id" value="<?php echo $_SESSION[$sys_id]["id"];?>">
                            ...
                            <div class="modal-dialog modal-dialog-scrollable">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body px-5">
                                        <label for="remark" class="form-check-label" >點檢備註說明：</label>
                            ...
                            <tbody>
                                ...
                                <?php foreach($div_checkeds as $checked_list){ ?>
                                    <tr>
                                        ...
                                        <td><?php echo $checked_list['site_title'];?></td>
                                        ...
                                        <td style="text-align: left;"><?php if(isset($checked_list['created_at'])){ 
                                                        echo $checked_list['created_at']." / ".$checked_list['cname'];}?></td>
                                        <td style="width:30%; text-align: left; word-break: break-all;"><?php echo $checked_list['remark'];?></td>
...
<script>
    ...
    var checkItem = document.getElementById("checkItem");   // 歷史清單裡的按鈕
