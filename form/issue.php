...
<body>
    <div class="col-12">
        <div class="row justify-content-center">
            <div class="col_xl_12 col-12 p-3 rounded" style="background-color: rgba(200, 255, 255, .6);" >
                <!-- 表頭 -->
                <div class="row">
                ...
                <div class="col-12 p-0">
                    <ul class="nav nav-tabs">
                        <li class="nav-item">
                            <a class="nav-link active" href="issue.php" aria-current="page" data-toggle="tooltip" data-placement="bottom" title="bar">
                ...
                <div class="col-12 bg-white">
                    ...
                    <div class="row">
                        ...
                        <div class="col-12 col-md-6 py-1 text-end">
                            <?php if($sys_role <= 2){ ?>
                                <a href="../<?php echo $form_type;?>/form.php?action=create" class="btn btn-primary"><i class="fa fa-edit" aria-hidden="true"></i> 填寫請購需求</a>
                            <?php } ?>
                            <?php if($sys_role <= 1){ ?>
                                <a href="../<?php echo $form_type;?>/show_issueAmount.php" title="管理員限定" class="btn btn-warning"><i class="fa-brands fa-stack-overflow"></i> 待轉PR總表</a>
                            <?php } ?>
                        </div>
                    </div>
                    <?php if($per_total > 0){ ?>
                            ...
                            <tbody>
                                <?php foreach($row_lists_div as $row){ ?>
                                    <tr>
                                        ...
                                        <td><?php $sys_role = ($sys_role <= 1) || (($row['fab_i_id'] == $sys_fab_id) || (in_array($row['fab_i_id'], $sys_sfab_id)));
                                            switch($row['idty']){
                                        ...
                                        <td>
                                            ...
                                            <?php if(($row['idty'] == '1') && ($sys_role <= 1)){ // 1待簽 ?>        
                                                <!-- 待簽：PM+管理員功能 -->
                                                <a href="../<?php echo $form_type;?>/show.php?id=<?php echo $row['id'];?>&action=sign" class="btn btn-sm btn-xs btn-primary">簽核</a>
                                                <!-- siteUser功能 -->
                                            <?php } else if(($row['idty'] == '11') && ($sys_role <= 1)){ ?>
                                                <a href="../<?php echo $form_type;?>/show.php?id=<?php echo $row['id'];?>&action=sign" class="btn btn-sm btn-xs btn-warning">待辦</a>
                                                <!-- siteUser功能 -->
                                            <?php } else if((in_array($row['idty'], [2, 13])) && (($row['fab_i_id'] == $sys_fab_id) || (in_array($row['fab_i_id'], $sys_sfab_id))) ){ ?>
                                                <a href="../<?php echo $form_type;?>/show.php?id=<?php echo $row['id'];?>&action=sign" class="btn btn-sm btn-xs btn-warning">待辦</a>
                                            <?php } else { ?>
                                                <!-- siteUser功能 -->
                                                <a href="../<?php echo $form_type;?>/show.php?id=<?php echo $row['id'];?>&action=review" class="btn btn-sm btn-xs btn-info">檢視</a>
                                            <?php }?>
                                        </td>
                                    </tr>
                                <?php } ?>
