...
// // // 查詢待簽名單for send MAPP
    function inSign_list(){
        ...
        $sql = "SELECT emp_id, cname
                    ...
                    , SUM(issue_waiting + receive_waiting) AS total_waiting , SUM(issue_3_count + receive_3_count) AS ppty_3_count
                FROM (
                        SELECT _i.in_sign AS emp_id, _i.in_signName AS cname
                            , COUNT(_i.in_sign) AS issue_waiting , 0 AS receive_waiting
                    ...
                    UNION ALL
                        SELECT _r.in_sign AS emp_id, _r.in_signName AS cname
                            , 0 AS issue_waiting , COUNT(_r.in_sign) AS receive_waiting
    ...
    function inCollect_list(){
        $pdo = pdo();
        // $sql = "SELECT _u.emp_id, _u.cname, _f.fab_title, _l.local_title, COUNT(_r.uuid) AS collect_count, SUM(CASE WHEN _r.ppty = 3 THEN 1 ELSE 0 END) AS ppty_3_count
        //         FROM _users _u
        ...
        //         GROUP BY _u.emp_id, _u.cname, _f.fab_title, _l.local_title
        //         ORDER BY _u.emp_id ASC ";
