                            ...
                            <div class="col-6 col-md-3 p-1">
                                <div class="col-12 rounded border bg-white" style="text-align: left;vertical-align:top;word-break: break-all">
                                    ...
                                    <div class="" style="height: 160px; text-align:center;">
                                        <input type="checkbox" name="catalog_id[<?php echo $catalog["id"];?>]" id="catalog_id_<?php echo $catalog["id"];?>" class="img unblock" value="<?php echo $catalog["id"];?>"
                                            <?php if(isset($low_level[$catalog['id']])){ echo "checked"; }?>>
                                        <label for="catalog_id_<?php echo $catalog["id"];?>"><img src="../catalog/images/<?php echo $catalog["cover"];?>" class="img-thumbnail"></label>
                                    </div>
                                    <!-- catalog info -->
                                    <div style="height: 100px;">
                                        ...
                                        <?php echo $catalog["id"].'.&nbsp'.$catalog["title"];?>
                                    </div>
                                    <!-- issue amount -->
                                    <div class="px-1">
                                        <?php switch($buy_ty){
                                        ...
                                        <div class="form-floating">
                                            <input type="number" name="amount[<?php echo $catalog['id'];?>]" id="<?php echo $catalog['id'];?>" class="form-control amount t-center" 
                                                placeholder="請填最低值" min="1" max="400" 
                                                    ...
                                                    value="<?php echo !empty($low_level[$catalog['id']]) ? $low_level[$catalog['id']]:$buy_qty ;?>">
                                            <label for="catalog_id_<?php echo $catalog["id"];?>"><?php echo "建議值：".$buy_qty."&nbsp/&nbsp".$catalog["unit"];?> </label>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<script>
    ...
    for(let amount of amounts){
        amount.onchange = e => {
            ...
            if(amount.value == ''){
                document.getElementById('catalog_id_'+ amount_id).checked=false;
            } else {
                document.getElementById('catalog_id_'+ amount_id).checked=true;
            }
        }
    }
    // setInterval(5000);
