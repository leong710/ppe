<?php
    ...
    // 1-1 將fab_id加入sfab_id
        if(isset($_SESSION[$sys_id]["fab_id"])){
            $fab_id = $_SESSION[$sys_id]["fab_id"];                                 // 1-1.取fab_id
        }else{
            $fab_id = "0";
        }
        if(!in_array($fab_id, $_SESSION[$sys_id]["sfab_id"])){                  // 1-1.當fab_id不在sfab_id，就把部門代號id套入sfab_id
            array_push($_SESSION[$sys_id]["sfab_id"], $fab_id);
        }
        $sfab_id = $_SESSION[$sys_id]["sfab_id"];                               // 1-1.取sfab_id
        $sfab_id = implode(",",$sfab_id);                                       // 1-1.sfab_id是陣列，要轉成字串
    // 1-2 組合查詢條件陣列
        $basic_query_arr = array(
            'sfab_id'   => $sfab_id,                                            // 1-2.將字串sfab_id加入組合查詢陣列中
            'fab_id'    => "allMy"
        );
    // 1-3 把我的fabs讀進來作為[篩選]的select option
        $fabs = show_fab($basic_query_arr);                                     // 篩選查詢清單用
    ...
    // 2-2 把fab_id篩選條件帶入查詢條件陣列
        $basic_query_arr['fab_id'] = $sort_fab_id;
    ...
    // 3-2 把cate_no篩選條件帶入查詢條件陣列
        $basic_query_arr['cate_no'] = $sort_cate_no;
 
        $stocks = show_stock($basic_query_arr);                  // 依查詢條件儲存點顯示存量
        $categories = show_categories();                         // 取得所有分類item
        $sum_categorys = show_sum_category($basic_query_arr);    // 統計分類與數量

        $sortFab = show_fab($basic_query_arr);                   // 查詢fab的細項結果
    ...
    // <!-- 20211215分頁工具 -->
        ...
        $div_stocks = show_stock($basic_query_arr);
                    ...
                    <div class="col-md-4 py-0">
                        <form action="" method="post">
                            <div class="input-group">
                                <select name="fab_id" id="sort_fab_id" class="form-select" onchange="this.form.submit()">
                                    <option for="sort_fab_id" value="All" <?php echo $sort_fab_id == "All" ? "selected":""; echo $_SESSION[$sys_id]["role"] >= 2 ? "hidden":""; ?>>-- [ All fab ] --</option>
                                    <option for="sort_fab_id" value="allMy" <?php echo $sort_fab_id == "allMy" ? "selected":"";?>>-- [ All my fab ] --</option>
                                    <?php foreach($fabs as $fab){ ?>
                                        <option for="sort_fab_id" value="<?php echo $fab["id"];?>" title="<?php echo $fab["fab_title"];?>" <?php echo $sort_fab_id == $fab["id"] ? "selected":"";?>>
